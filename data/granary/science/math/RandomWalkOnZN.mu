...
至少在具有高对称性的情况下，我们不难将上述随机运动分解，分解所用的技巧在证明Polya回归定理中也会用到，而起结果便是任意D维随机运动都可以分解为D个1维随机运动的直积，而一维情况下随机运动的结果是布朗运动，这是爱因斯坦在其著名的关于布朗运动的论文中所证明了的，因此D维离散空间中的随机运动实际上也就是[D维布朗运动](https://en.wikipedia.org/wiki/Brownian_motion)。连续时并不是单纯的D维布朗运动的原因在于，每一步的增量分布并不满足正态分布。

至此，在N维空间$\mathbb{Z}^N$中的随机行走问题也就有了答案：它就是一个离散空间上的热扩散问题，而离散空间中则是布朗运动。