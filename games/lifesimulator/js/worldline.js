(() => {
	const AllMen = ['man1', 'man2', 'man3'];
	const chooseNext = (type, choise) => status => {
		var range = [...AllMen];
		...
		if (range.length === 0) {
			...
			next = range[Math.floor(range.length * Math.random())];
		}
		return {goto: next};
	};
	window.WorldLine = {
		events: {
			...
			man3: {
				...
				choise: [
					{
						hint: "恶心！滚粗！",
						goto: chooseNext('man3', false),
						points: {man: -1, man3: 1, zhonger: -1},
						result: "被你抛弃后，他发愤图强，最终成为了举世闻名的，漫画家。"
					},
					{
						hint: "啊！好可爱啊！好想捏捏啊！",
						goto: chooseNext('man3', true),
						points: {money: -1, man3: 1, zhonger: 1},
