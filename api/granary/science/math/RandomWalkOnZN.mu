...
但有趣的是，我们会发现这个连接因子实际上只是维数$N$的函数，而和到底有多少连接无关：$\omega = \frac{1}{2 N}$。比如，六边形时候求和部分会给出一个因子$\frac{3}{2}$，但连接数$n = 6$，于是结果依然是$\frac{3}{2}\frac{1}{6} = \frac{1}{4}$。甚至考虑可以在单位元上连续取值的情况：$\frac{1}{2 \pi} \int_0^\pi \sin(x)^2 dx = \frac{1}{4}$。
